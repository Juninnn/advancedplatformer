...
func _physics_process(delta: float):
	...
	if player_sprite != null:
		...
		print("Erro: player_sprite não está inicializado.")
...
func vertical_movement_env() -> void:
	...
	if Input.is_action_just_pressed("jump") and jump_count < 2 and jump_condition:
		print("Jumping!")  # Debugging line
		jump_count += 1
		velocity.y = -jump_speed  # Ensure this is negative for upward movement
''
...
func defense() -> void:
	if Input.is_action_just_pressed("defend") and is_on_floor() and not crouching:
		defending = true
		can_track_input = false
	elif not crouching:
		defending = false
